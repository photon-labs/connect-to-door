...
import com.maretska.attendance.FingerIdManager;
 ...
 */
@Controller
@RequestMapping("/verify-finger-id")
public class FingerIdService {

	private static final String FINGER_ID = "finger_id";
	...
	public @ResponseBody String loginAttendance(@RequestBody String entity, HttpServletResponse servletResponse){
		...
		FingerIdManager fingerIdManager = new FingerIdManager();

		try {
			JSONObject request = new JSONObject(entity);

			if(request.has(FINGER_ID)){
				fingerId = request.getString(FINGER_ID);

				jsonObject = fingerIdManager.getEmployeeId(fingerId);
			}else{
